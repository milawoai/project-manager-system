...
const PLATFORM_LABEL_MAP = new Map<number, string>(PLATFORM_OPTIONS.map((item) => [item.code, item.label]))

export function getPlatformLabel(code: number | string | null | undefined): string {
  const parsed = typeof code === 'string' ? Number(code) : code
  if (parsed === null || parsed === undefined || Number.isNaN(parsed)) return '-'
  return PLATFORM_LABEL_MAP.get(parsed) || '-'
}
